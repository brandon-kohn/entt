...
class basic_sparse_set {
    ...
    [[nodiscard]] auto assure_page(const std::size_t pos) {
        if(!(pos < bucket)) {
            const size_type sz = pos + 1u;
            auto mem = bucket_alloc_traits::allocate(bucket_allocator, sz);

            std::uninitialized_move(sparse, sparse + bucket, mem);
            std::uninitialized_fill(mem + bucket, mem + sz, alloc_pointer{});
        ...
        if(!sparse[pos]) {
            ...
            std::uninitialized_fill(sparse[pos], sparse[pos] + page_size, null);
        }
    ...
    void resize_packed(const std::size_t req) {
        ENTT_ASSERT(!(req < count), "Requested size cannot be less than count");
        auto old = std::exchange(packed, alloc_traits::allocate(allocator, req));
        std::uninitialized_move(old, old + count, packed);
        std::destroy(old, old + count);
    ...
    void pop(const Entity entt, void *ud) {
        ...
        packed[pos] = std::exchange(packed[--count], entt);
    ...
    void clear_sparse() {
        if(bucket) {
            for(size_type next{}; next < bucket; ++next) {
                if(sparse[next]) {
                    std::destroy(sparse[next], sparse[next] + page_size);
                    alloc_traits::deallocate(allocator, sparse[next], page_size);
                }

                bucket_alloc_traits::destroy(bucket_allocator, std::addressof(*(sparse + next)));
            }
    ...
    void clear_packed() {
        if(reserved) {
            ...
            alloc_traits::deallocate(allocator, packed, std::exchange(reserved, 0u));
        }
    }
